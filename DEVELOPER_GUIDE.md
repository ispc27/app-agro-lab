...
|---|---|---|---|---|
| **HU-01 (Objetivo 1)** | Clientes Frecuentes | `frequent_clients_churn.py` | `frequent_clients_churn/` | • Ranking interactivo de clientes ordenados por volumen de muestras (descendente).<br/>• Filtros dinámicos por rango de fechas, estado de actividad (Activos/Inactivos con criterio de 90 días), **filtro multi-especie** y **perfil agronómico (Estacional vs. Mixto)**.<br/>• Tarjeta de Cliente Líder por ID y tabla detallada con recencia en días. |
| **HU-02 (Objetivos 2 y 3)** | Cultivos y Capacidad | `crop_capacity.py` | `crop_capacity/` | • Distribución por especie (Pareto Top 10 + "Otras") con alternancia Barras/Dona.<br/>• **Gráfico 1: Capacidad Operativa y Alertas del Laboratorio**: Volumen consolidado (todas las especies) con umbrales fijos en 142 (Alerta Operativa) y 191 (Cuello de Botella).<br/>• **Gráfico 2: Patrón de Estacionalidad (Trigo y Soja)**: Curvas continuas sin alertas para analizar zafras y picos de demanda.<br/>• Nota: Desestimación justificada del desglose de ensayos por inconsistencia en `tipo_analisis`. |
| **HU-03 (Objetivo 4)** | Segmentación RFM | `rfm_segmentation.py` | `rfm_segmentation/` | • Scoring RFM de 3 dígitos (111 al 555) sobre toda la cartera (sin exclusión ID > 50.000).<br/>• Clasificación en **5 categorías oficiales**: Campeones, Fieles / Alto Valor, Potenciales, En Riesgo y Perdidos.<br/>• **Listado de Contactos Urgentes (En Riesgo)** para retención comercial inmediata.<br/>• Distribución de fracciones de cartera (Barras/Dona) y Matriz RFM 2D. |
